...
namespace cindex
{
	...
	public:
		...
		{
			assert(static_cast<std::size_t>(static_cast<ref_type>(num_symbols_ * 2 - 1))
					== num_symbols_ * 2 - 1);
		...
		std::size_t symbol(ref_type ref) const  { return static_cast<std::size_t>(ref); }

		ref_type root()  { return static_cast<ref_type>(num_symbols_ * 2 - 2); }
		const ref_type root() const  { return static_cast<ref_type>(num_symbols_ * 2 - 2); }
	...
	public:
		...
		FreqType& operator[](std::size_t symbol)  { assert(symbol < num_symbols_); return freqs_[symbol]; }

		const FreqType& operator[](std::size_t symbol) const  { assert(symbol < num_symbols_); return freqs_[symbol]; }
	...
	public:
		...
		std::size_t length(std::size_t symbol) const  { assert(symbol < num_symbols_); return lengths_[symbol]; }
		const block_type* code(std::size_t symbol) const  { assert(symbol < num_symbols_); return codes_[symbol]; }
	...
	public:
		...
		ref_type symbol(std::size_t v) const  { assert(v < (1u << max_length_)); return refs_[v]; }
		std::size_t length(std::size_t v) const  { assert(v < (1u << max_length_)); return lengths_[v]; }
