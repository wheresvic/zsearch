...
//  $Id: cedarpp.h 1739 2014-01-28 15:09:06Z ynaga $
// Copyright (c) 2009-2013 Naoki Yoshinaga <ynaga@tkl.iis.u-tokyo.ac.jp>
#ifndef CEDAR_H
#define CEDAR_H
...
namespace cedar {
  ...
  public:
    ...
    struct block { // a block w/ 256 elements
      ...
      int  num;    // # empty elements; 0 - 256
      int  trial;  // # trial
      int  ehead;  // first empty item
      block () : prev (0), next (0), num (256), trial (0), ehead (0) {}
    };
    da () : tracking_node (), _array (0), _tail (0), _tail0 (0), _ninfo (0), _block (0), _bheadF (0), _bheadC (0), _bheadO (0), _capacity (0), _size (0), _quota (0), _quota0 (0), _no_delete (false)
    { _initialize (); }
    ~da () { clear (false); }
    ...
    value_type& update (const char* key, npos_t& from, size_t& pos, size_t len, value_type val = value_type (0)) {
      ...
      if (! offset) { // node on trie
        for (const uchar* const key_ = reinterpret_cast <const uchar*> (key);
          ...
          if (pos == len)
            { const int to = _follow (from, 0); return _array[to].value += val; }
      ...
      if (offset >= sizeof (int)) { // go to _tail
        ...
        if (from >> 32) {
          ...
          for (npos_t offset_ = static_cast <npos_t> (-_array[from].base);
            ...
            from = static_cast <size_t>
                   (_follow (from, static_cast <uchar> (_tail[offset_])));
            ++offset_;
        ...
        for (size_t pos_ = pos_orig; pos_ < pos; ++pos_)
          from = static_cast <size_t>
                 (_follow (from, static_cast <uchar> (key[pos_])));
        npos_t moved = pos - pos_orig;
        if (tail[pos]) { // remember to move offset to existing tail
          const int to_ = _follow (from, static_cast <uchar> (tail[pos]));
        ...
        from = static_cast <size_t> (_follow (from, static_cast <uchar> (key[pos])));
        ++pos;
      }
    ...
    int save (const char* fn, const char* mode = "wb", const bool shrink = false) { // const
      // _test ();
    ...
    int begin (npos_t& from, size_t& len) {
      ...
      from &= TAIL_OFFSET_MASK;
      from |= static_cast <npos_t> (-base + len_) << 32;
      len += len_;
    ...
    int next (npos_t& from, size_t& len, const npos_t root = 0) {
      uchar c = 0;
      if (const int offset = from >> 32) { // on tail
      ...
      return begin (from = _array[from].base ^ c, ++len);
    }
  ...
  private:
    ...
    // follow/create edge
    int _follow (npos_t& from, const uchar& label) {
      int to = 0;
    ...
    int _find_place (const uchar* const first, const uchar* const last) {
      if (int bi = _bheadO) {
        ...
        while (1) { // set candidate block
          block& b = _block[bi];
          if (b.num >= last - first + 1) // explore configuration
            for (int e = b.ehead;;) {
    ...
    int _resolve (npos_t& from_n, const int base_n, const uchar label_n) {
